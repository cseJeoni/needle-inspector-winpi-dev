...
def find_available_cameras(limit=10):
    ...
    for device in dino_devices:
        debug_print(f"  - {device.get('Name', 'Unknown')}")
    ...
    try:
        ...
        # Dino 카메라가 자주 있는 인덱스부터 검색
        # 웹캠은 보통 0번, Dino는 더 뒤쪽에 있음
        # 0번은 아예 제외 (웹캠일 가능성 높음)
        ...
        for idx in range(limit):
            try:
                ...
                if cap_test.isOpened():
                    # OpenCV에서 카메라 이름 가져오기 (Windows에서는 작동 안할 수 있음)
                    backend_name = cap_test.getBackendName()
                    device_names[idx] = f"Camera_{idx}"
                    debug_print(f"[DEBUG] 인덱스 {idx}: 사용 가능한 카메라")
                    cap_test.release()
            except:
                pass
        ...
        for index in test_order:
            ...
            try:
                ...
                # 2. 밝기: 30-150 사이 (범위 확대)
                is_dino_resolution = (width == 640 and height == 480) or \
                                   (width == 1280 and height == 960) or \
                                   (width == 1280 and height == 1024)
                
                # 밝기 범위를 더 넓게 설정 (환경에 따라 다를 수 있음)
                is_dino_brightness = 30 < mean_brightness < 150
                
                if not is_dino_resolution:
                    debug_print(f"[SKIP] 카메라 인덱스 {index}: Dino 해상도 아님 ({width}x{height})")
                    continue
                    
                if not is_dino_brightness:
                    debug_print(f"[WARN] 카메라 인덱스 {index}: 밝기 범위 벗어남 ({mean_brightness:.1f}) - 그래도 포함")
                    # 해상도가 맞으면 밝기와 관계없이 포함
