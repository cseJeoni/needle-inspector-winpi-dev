...
function registerIpcHandlers() {
  ...
  // Auto Exposure Target 조회 핸들러
  ipcMain.handle('camera-ae-get-target', async (event, deviceIndex) => {
    try {
      const backendPath = getBackendPath();
      const exePath = path.join(backendPath, 'dist', 'camera_ae_control.exe');
      const scriptPath = path.join(backendPath, 'camera_ae_control.py');

      console.log(`[INFO] AE Target 조회: device=${deviceIndex}`);

      const args = ['get-target', '--device-index', deviceIndex.toString()];
      let stdout, stderr;
      ...
      if (stderr) {
        console.warn('[WARN] 카메라 AE Target 조회 경고:', stderr);
      }

      const result = JSON.parse(stdout.trim());
      return result;
    } catch (error) {
      console.error('[ERROR] 카메라 AE Target 조회 실패:', error);
      return {
        success: false,
        error: `카메라 AE Target 조회 실패: ${error.message}`
      };
    }
  });

  // Auto Exposure Target 설정 핸들러
  ipcMain.handle('camera-ae-set-target', async (event, deviceIndex, aeTarget) => {
    try {
      const backendPath = getBackendPath();
      const exePath = path.join(backendPath, 'dist', 'camera_ae_control.exe');
      const scriptPath = path.join(backendPath, 'camera_ae_control.py');

      console.log(`[INFO] AE Target 제어: device=${deviceIndex}, target=${aeTarget}`);

      const args = ['set-target', '--device-index', deviceIndex.toString(), '--ae-target', aeTarget.toString()];
      let stdout, stderr;
      ...
      if (stderr) {
        console.warn('[WARN] 카메라 AE Target 제어 경고:', stderr);
      }

      const result = JSON.parse(stdout.trim());
      return result;
    } catch (error) {
      console.error('[ERROR] 카메라 AE Target 제어 실패:', error);
      return {
        success: false,
        error: `카메라 AE Target 제어 실패: ${error.message}`
      };
    }
  });
